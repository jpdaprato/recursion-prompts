...
O: integer (num of times key appears in obj)
...
STRATEGY: recursion
  base case: val is not a nested obj
*/

let countKeysInObj = (obj, target, count = 0) => {
  for (let key in obj) {
    if (key === target) {
      count ++;
...
let countValuesInObj = (obj, value, count = 0) => {
  for (let key in obj) {
    if (obj[key] === value) {
      count ++;
    } else if (typeof obj[key] === 'object' && obj[key] !== null) {
...
STRATEGY: recursion
  base case: keys have been traversed and value at key is not an obj
*/
...
/*
I: array of lower-case strings

O: array of same strings capitalized
...
STRATEGY: recursion with closure
  base case: reached final index in array
...
let capitalizeWords = (array) => {
  ...
  let currentIndex = array[array.length - 1];
  array[currentIndex] = array[currentIndex].toUpperCase();
